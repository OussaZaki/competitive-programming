...
bool fruitInRange(int s, int t, int p) {
    // check if a fruit position (p) is in the range [s, t] i.e. s <= p <= t
    return s <= p && p <= t;
}

// Complete the countApplesAndOranges function below.
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int apples_in = 0, oranges_in = 0;
    for (int apple_i : apples)
    {
        apples_in += fruitInRange(s, t, a + apple_i) ? 1 : 0;
    }
    
    for (int orange_i : oranges)
    {
        oranges_in += fruitInRange(s, t, b + orange_i) ? 1 : 0;
    }

    cout << apples_in << endl;
    cout << oranges_in << endl;
}
